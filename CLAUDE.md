...
Semantics to remember:
- **Body budget resolution:** explicit `bodyLimit` -> derived `maxFileSize × maxFiles + fieldsBudget` when both are finite -> `@BodyLimit` on the method or the controller -> nothing of its own, in which case the route's own body limit (its `@BodyLimit`, else the app default) is what a chunked body is counted against on node-server, and the adapter ceiling everywhere else. The winner is written into `BODY_LIMITS`, so `@Multipart` and `@BodyLimit` on the same method fight over one slot (last decorator applied wins) - do not combine them. Whatever the route ends up with also raises `Router.adapterBodyCeiling` for the **whole app** (`max(default, all route limits)`), so a generous upload route quietly lifts the 1MB default for every other route.
...
- **The body is consumed once:** `@Multipart` is incompatible with `@ValidateBody` / `ctx.json()` / `ctx.text()` on the same route. A non-multipart request is rejected with 400 before the handler runs, whether or not it touches `ctx.parts`. `ctx.parts` and `ctx.form()` exclude each other too, and the package enforces it - `ctx.parts` is handed out wrapped, so whichever runs second throws instead of returning an empty form or iterating zero times.
...
**Body size limits.** `new Miia({ maxBodySize })` (default 1MB, `false` disables) + per-route `@BodyLimit(bytes)` (method > class > app option). Limits are resolved into the route table at registration time; declared Content-Length is checked in core after route matching -> `PayloadTooLargeException` (413). Chunked bodies carry no length to check, so they are counted as they arrive. After route matching `checkBodyLimit()` narrows the adapter's `_bodyLimit` slot to the **route's** limit (`Math.min`, never widening what the adapter set) - node-server owns that slot and applies it lazily when the body is first materialized, so the route's own limit is what a chunked body is judged against there. Everything without the slot keeps the adapter ceiling, `max(maxBodySize, all @BodyLimit values)`: Bun (`maxRequestBodySize`), Deno (counting stream wrapper in `listen()`), uws-server (in-stream cap), both adapters in `mode: 'native'`, and anything reached through `app.fetch` including `TestApp`. Adapter streams error with an `Error` named `'PayloadTooLargeError'` since adapters don't depend on core. Enforcement is consumption-time: a handler that never reads a chunked body lets the client upload up to the ceiling regardless.
