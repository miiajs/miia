...
When `app.destroy()` runs (SIGTERM, deploy, restart), each transport waits up to `drainTimeoutMs` (default `5000`) for in-flight handlers to finish before tearing down state. Without drain, a handler interrupted mid-execution leaves the message un-acked - it stays pending on the broker until something redelivers it, and the work runs twice. On the Redis transport that redelivery comes from a housekeeping pass on any replica, once the entry crosses the [retry horizon](/docs/packages/messaging/redis#the-horizon).
