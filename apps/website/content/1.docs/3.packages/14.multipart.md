...
Backpressure is real on the streaming path: the request body is only read further when every queue the handler can still drain is empty, so a slow storage target never piles parts up inside the handler. How far back that reaches - whether it slows the client down as well, or only bounds what the handler holds - is the server's call, and it differs between the adapters underneath.
...
Nothing here writes to a filesystem, and the code that does is short enough to keep in the handler - but four details in it are worth stating outright. The `uploads` module in `examples/full-app` has the whole thing working end to end.

- **Bridge the part with `Readable.from(part.stream)`, not `Readable.fromWeb`.** On Bun 1.3.12 `fromWeb` never settles once the web stream errors, so a failed upload leaves the request hanging forever. `Readable.from` behaves on all three runtimes.
...
4. Otherwise nothing. The route gets no multipart budget of its own, and the adapter's ceiling is the only thing left.

Steps 1 and 2 also write the route's body limit, so a body that declares an oversized `Content-Length` is rejected by the router before the parser ever starts. Bodies without a declared length - chunked uploads - are counted by the parser as the bytes arrive under steps 1, 2 and 3. Step 4 leaves the parser without a budget to count against, so a chunked body on a bare `@Multipart()` is bounded only by the adapter's ceiling - which is `max(maxBodySize, the largest route limit in the app)`, not the `maxBodySize` you set.
...
- **How far backpressure reaches back is the server's call.** The parser reads the body only as fast as the handler drains it, but whether that throttles the client too depends on the server underneath, and the adapters differ. The limits hold everywhere; only the flow control differs.
- **A derived body limit lifts the app-wide ceiling.** The router raises the adapter's body ceiling to the largest route limit registered, so a route deriving `maxFileSize * maxFiles + fieldsBudget` quietly relaxes the default for every other route as well. Only the ceiling moves; each route keeps its own limit.
