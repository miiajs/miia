...
::tip
You do not have to write `isRateLimited` yourself - [`@miiajs/rate-limit`](/docs/packages/rate-limit) ships a ready `rateLimit()` middleware plus a guard flow with `@RateLimit` / `@SkipRateLimit` decorators.
::
