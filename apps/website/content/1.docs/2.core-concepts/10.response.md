...
`cookie()` and `deleteCookie()` write response cookies. See [Cookies](/docs/core-concepts/cookies) for the full cookie API, including reading incoming cookies via `ctx.cookies`.
