...
::note
`PayloadTooLargeException` is also thrown automatically by the framework when a request body exceeds the configured limit - see [Body size limits](/docs/core-concepts/controllers#body-size-limits). `TooManyRequestsException` is thrown automatically by [`@miiajs/rate-limit`](/docs/packages/rate-limit) when a client exceeds a rate limit.
::
