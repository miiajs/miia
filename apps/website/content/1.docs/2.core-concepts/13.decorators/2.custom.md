...
- **`packages/swagger/src/builder/spec-builder.ts`** - the Swagger spec builder reads core metadata (`ROUTES`, `CLASS_GUARDS`, `SKIP_GUARDS`, `STATUSES`, `BODY_SCHEMAS`, `QUERY_SCHEMAS`, `PARAMS_SCHEMAS`) **plus** its own custom keys (`API_TAG`, `API_OPERATIONS`, `API_RESPONSES`, `API_SECURITY`, `API_HEADERS`) to generate an OpenAPI 3.1 document. It also reads `GUARD_RESPONSES` - a symbol-keyed static on the guard class rather than `Symbol.metadata` state, since a guard declares its own rejection codes and no decorator is involved.
