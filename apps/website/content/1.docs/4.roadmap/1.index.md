...
Current package state, all on `0.2.0`:

| Package | Status | Tests | Note |
| --- | --- | --- | --- |
| `@miiajs/core` | beta | 220 | Foundation. DI, decorators, router, middleware, exceptions. |
...
| [`@miiajs/rate-limit`](/docs/packages/rate-limit) | beta | 54 | Fixed-window rate limiting. Guard + decorators (`@BodyLimit`-style precedence), perimeter middleware, pluggable stores. |
...
- [**Short term (0-3 months)**](/docs/roadmap/short-term) - hardening: validate `experimental` packages on real apps, file upload, rate-limit stores (Redis, Upstash), server-sent events.
...
2. **Production essentials missing from existing packages.** Multipart, SSE, distributed rate-limit stores. These unblock real apps for real deployments.
