...
Current package state, all on `0.6.0`:

| Package | Status | Tests | Note |
| --- | --- | --- | --- |
| `@miiajs/core` | beta | 259 | Foundation. DI, decorators, router, middleware, exceptions. |
| [`@miiajs/node-server`](/docs/packages/node-server) | beta | 38 | Optimized Node.js HTTP adapter. |
| [`@miiajs/uws-server`](/docs/packages/uws-server) | beta | 38 | uWebSockets.js adapter, parallel optimization to node-server. |
| [`@miiajs/swagger`](/docs/packages/swagger) | beta | 89 | OpenAPI 3.1 + Swagger UI. |
| [`@miiajs/messaging`](/docs/packages/messaging) | beta | 81 | Decorator-driven event bus, retry, DLQ, idempotency, named buses. |
| `@miiajs/config` | beta | 16 | Zod-validated env via `ConfigService` + `ConfigModule.configure`. |
| [`@miiajs/rate-limit`](/docs/packages/rate-limit) | beta | 55 | Fixed-window rate limiting. Guard + decorators (`@BodyLimit`-style precedence), perimeter middleware, pluggable stores. |
| [`@miiajs/multipart`](/docs/packages/multipart) | beta | 86 | Streaming `multipart/form-data`: backpressured part iterator, buffered `ctx.form()`, `@ValidateForm`. Differential tests diff it against the runtime's own parser across the corpus's chunk-boundary cut points. |
| [`@miiajs/papr`](/docs/packages/papr) | beta | 26 | MongoDB via Papr. |
| [`@miiajs/mongoose`](/docs/packages/mongoose) | beta | 29 | MongoDB via Mongoose. |
| [`@miiajs/drizzle`](/docs/packages/drizzle) | beta | 18 | Drizzle ORM (PG/MySQL/SQLite). |
| [`@miiajs/auth`](/docs/packages/auth) | beta | 13 | Strategy primitives, AuthGuard. Small surface; critical paths covered. |
| [`@miiajs/jwt`](/docs/packages/jwt) | beta | 6 | JWT sign/verify (jose wrapper). Core logic tested upstream in `jose`. |
| [`@miiajs/serve-static`](/docs/packages/serve-static) | beta | 51 | Static file serving. Full HTTP semantics (Range, conditional GET, HEAD, dotfile guard, SPA fallback). |
| [`@miiajs/cli`](/docs/packages/cli) | experimental | 51 | Build/dev/scaffold CLI. Scaffolds not yet validated by real adoption. |
| [`@miiajs/messaging-redis`](/docs/packages/messaging/redis) | experimental | 44 | Redis Streams transport + idempotency store. Retry lives in the consumer group's PEL, so crash recovery is the same code path as a backoff retry and the suite covers it directly. Untested at production load. |
...
- [**Short term (0-3 months)**](/docs/roadmap/short-term) - hardening: validate `experimental` packages on real apps, rate-limit stores (Redis, Upstash), server-sent events.
...
2. **Production essentials missing from existing packages.** SSE, distributed rate-limit stores. These unblock real apps for real deployments. Multipart came off this list in 0.6.
